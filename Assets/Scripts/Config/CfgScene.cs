...
using System.Collections.Generic;
    ...
    {
        private readonly Dictionary<int, RowCfgScene> _configs = new Dictionary<int, RowCfgScene>(); //cfgId映射row
        public RowCfgScene this[Enum cid] => _configs.ContainsKey(cid.GetHashCode()) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
        public RowCfgScene this[int cid] => _configs.ContainsKey(cid) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
