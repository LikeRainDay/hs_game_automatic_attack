...
using System.Collections.Generic;
    ...
    {
        private readonly Dictionary<int, RowCfgPrefab> _configs = new Dictionary<int, RowCfgPrefab>(); //cfgId映射row
        public RowCfgPrefab this[Enum cid] => _configs.ContainsKey(cid.GetHashCode()) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
        public RowCfgPrefab this[int cid] => _configs.ContainsKey(cid) ? _configs[cid.GetHashCode()] : throw new Exception($"找不到配置 Cfg:{GetType()} configId:{cid}");
