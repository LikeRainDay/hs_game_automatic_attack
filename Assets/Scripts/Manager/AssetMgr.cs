...
public static class AssetMgr
{
    ...
    public static async Task<T> LoadAssetAsync<T>(string path, AssetLoadType assetLoadType = AssetLoadType.Permanent) where T : Object
    {
        ...
        path = ReplacePath(path);//将路径修改为当前语言当前画质的
    ...
    public static void ReleaseByLoadType(AssetLoadType assetLoadType = AssetLoadType.Temp)
    {
        //如果之前字典里还不存在这种加载类型 就初始化一下键值对
        if (!m_LoadTypeToHandleDict.ContainsKey(assetLoadType))
        {
    ...
    /// 将路径自动修改为当前语言当前画质的
    /// </summary>
    /// <param name="oldPath">初始的路径</param>
    /// <returns></returns>
    public static string ReplacePath(string oldPath)
    {
        //路径带有4K的资源才会去区分124K
        if (oldPath.Contains("4K"))
        {
            //如果当前分辨率是1K或者2K，就将路径修改成1K或者2K，否则不变即可
            switch (GameManager.Instance.CurDefinition)
            {
                case 1:
                    oldPath = oldPath.Replace("4K", "1K");
                    break;
                case 2:
                    oldPath = oldPath.Replace("4K", "2K");
                    break;
            }
        }

        //同理路径带有_cn的资源才需要去区分语言
        if (oldPath.Contains("_cn"))
        {
            switch (GameManager.Instance.CurLanguage)
            {
                case Language.English:
                    oldPath = oldPath.Replace("_cn", "_en");
                    break;
                case Language.Japanese:
                    oldPath = oldPath.Replace("_cn", "_jp");
                    break;
                default:
                    break;
            }
