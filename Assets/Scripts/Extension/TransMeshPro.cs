...
/// *63多语言MeshPro文本组件
/// </summary>
public class TransMeshPro : TextMeshProUGUI, IInit
{
    ...
    public float CurrentFontWidth => m_xAdvance; //当前行的字符宽度，有时需要根据内容长度来确定位置或者组件的size
    private bool m_IsDirty; //脏标记
    private float m_CacheFontSize; //初始字体大小
    private float m_CacheLineSpacing; //初始行间距
    private float m_CacheCharacterSpacing; //初始字间距
    ...
    /// 更新翻译id
    /// </summary>
    /// <param name="transContentList">填充占位坑位的数据</param>
    ...
    private void UpdateText()
    {
        ...
        foreach (var transContent in m_TransContentList)
        {
            ...
            //print(m_ResultBuilder.ToString());
        }
    ...
    {
        //获取字体
        font = await LanguageDef.GetFont<TMP_FontAsset>(FontSettingId, font.name);

        //如果缺少配置 则使用默认字体配置
        if (SettingId == 0) SettingId = 1;

        var rowCfgFontSetting = ConfigManager.Instance.cfgFontSettingPro[SettingId];
        var languageId = GameManager.Instance.CurLanguage.GetHashCode();
        //越界
        if (languageId >= rowCfgFontSetting.meshProSettings.Length)
        {
        ...
        //调整新字体参数
        fontSize = m_CacheFontSize * rowCfgFontSetting.meshProSettings[languageId].fontSizeScale;
        lineSpacing = m_CacheLineSpacing * rowCfgFontSetting.meshProSettings[languageId].lineSpacingScale;
        characterSpacing = m_CacheCharacterSpacing * rowCfgFontSetting.meshProSettings[languageId].characterSpacingScale;
        m_IsDirty = true;
        UpdateText();
    }
    ...
    public void Init(params object[] objs)
    {
        ...
        m_CacheCharacterSpacing = characterSpacing;
